...
fs.readFile('./auth/key.json', function processClientSecrets(err, content) {
  if (err) {
  ...
  // Drive API.
  authorize(JSON.parse(content), saveFile );  //!HERE@  listFiles
});
...
function saveFile( auth ){
	...
	queue.reduce(function(sequence,filename){
		...
		return sequence.then(function(){
			...
			return uploadFile( auth, filename);


		}).then(function(d){
			
			console.log("Complete with response: ");
			console.log(d)
